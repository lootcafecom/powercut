      ...
      <div className="flex gap-1 border-b border-line" role="tablist">
        {tabs.map((tab) => (
          <button
            ...
            className={`px-4 py-2.5 text-sm font-semibold uppercase tracking-wide transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-signal ${
              active === tab.key
          ...
          >
            {tab.label}
            <span className="ml-1.5 tabular-nums-mono text-xs text-muted">
              {data[tab.key].length}
            </span>
          </button>
        ))}
